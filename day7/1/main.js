...
function addDirectorySize(size, dir) {
	if (dir.getSize() <= 100000) size += dir.getSize();
	dir.getDirectories().forEach((child) => {
		size = addDirectorySize(size, child);
	});
	return size;
}

console.log(addDirectorySize(output, root));
